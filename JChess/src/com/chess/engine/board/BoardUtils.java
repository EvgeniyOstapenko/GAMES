...
public class BoardUtils {
    ...
    private static boolean[] initRow(int rowNumber) {
        final boolean[] row = new boolean[NUN_TILES];
        do {
            row[rowNumber] = true;
            rowNumber++;
        } while (rowNumber % NUN_TILES_PER_ROW != 0);
        return row;
    }

    public static final int NUN_TILES = 64;
    public static final int NUN_TILES_PER_ROW = 8;
    ...
    private static boolean[] initColumn(int columnNumber) {
        final boolean[] column = new boolean[NUN_TILES];

        do {
            column[columnNumber] = true;
            columnNumber += NUN_TILES_PER_ROW;
        } while (columnNumber < NUN_TILES);
